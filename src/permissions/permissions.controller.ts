...
@Controller("product")
export class ProductPermissionsController {
  private readonly logger = new Logger(ProductPermissionsController.name);

  constructor(private readonly permissionsService: PermissionsService) {}

  @Get("permissions")
  async findAll() {
    this.logger.log(`[REQUEST] GET /product/permissions`);

    const startTime = Date.now();
    const result = await this.permissionsService.findAllNonSaasAdmin();
